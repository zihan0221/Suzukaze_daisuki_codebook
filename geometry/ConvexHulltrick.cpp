...
(run convex_hull() before pass in) */
struct Convex {
  #ifndef all
  ...
  int PtSide(Pt p, Line L) {
    return dcmp(L.v ^ (p - L.s));
  }
  int inside(Pt p,
    const vector < Pt > & h, auto f) {
  ...
  // ret 0: out, 1: on, 2: in
  int inside(Pt p) {
    return min(inside(p, L, less<Pt>()), inside(p, U, greater<Pt>()));
  }
  ...
  array < int, 2 > tangent2(Pt p) {
    ...
    if (auto it = lower_bound(all(U), p, greater<Pt>()); it != U.end() and p == * it) {
  ...
  vector < int > intersect(Line L) {
    ...
    if (PtSide(A[l], L) == 0) return {
      l
    };
    if (PtSide(A[r], L) == 0) return {
      r
    };
